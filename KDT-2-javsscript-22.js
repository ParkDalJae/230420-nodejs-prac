...
function exampleOne(movies, minRating) {
  ...
  //init() 함수는 그 자체로 객체이므로 반환값을 바로 변수에 할당할 수 있다.

  //필터링된 영화, 정렬된 영화 및 상위영화 변수관리
  //빈 배열을 초기화(initialize)했기 때문에 개발자들 사이에서도
  //이 자리에 무슨 데이터가 들어갈지 예상하기 쉽다.
  //함수의 특징때문에 반환되는 객체는 추가되거나 삭제될 수 없는 불변성을 보장한다.
  function init() {
    return {
      filteredMovies: [],
      sortedMovies: [],
  ...
  //상위 영화 랭킹 로직 함수 최소 등급 이상 영화 중에 상위 삼위 반환
  function getTopMovies(movies, minRating) {
    ...
    //위처럼 하나씩 할당하는 대신, 아래처럼 비구조화 할당(destructuring assignment)을 사용 할 수도 있다.
